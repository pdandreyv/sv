@foreach ($categories as $category)
    ...
    @endif
            ...
            @foreach ($category->resources as $resource)
                <div class="resource">
                    <div class="label-check">
                        ...
                        />
                        <label for="{{'resource_'.$resource->id.'_'.$resource->category_id}}" class="post_checkbox_label"><span>{{$resource->name}} ({{ $resources_units[$resource->id] }})</span></label>
                    </div>
        ...
        @if ($category->children->count() > 0)
            @include('resources.categories', ['categories' => $category->children, 'resources_checked' => $resources_checked])
        @endif
    </div>
@endforeach